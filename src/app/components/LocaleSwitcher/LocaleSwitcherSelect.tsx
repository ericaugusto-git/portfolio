...
export default function LocaleSwitcherSelect({
  ...
  return (
    <label
      className={clsx(
        'relative  flex items-center align-middle cursor-pointer hover:text-slate-200',
        isPending && 'transition-opacity [&:disabled]:opacity-30'
      )}
    >
      <p className="sr-only">{label}</p>
      <div style={{maskImage: `url("${globe.src}")`}} className="block w-4 h-4 fill-white bg-white svgMask absolute top-[1px]"></div>
      <select
        ref={selectRef}
        style={{direction: 'rtl'}}
