...
export default function Projects() {
    ...
    const projects = [
        ...
        {name: 'Discord Clone', isOnline: true, img: '/images/discord_clone.png', link: 'https://discourse-live-chat.onrender.com', description:t('disc'), skills: ['next', 'tailwind', 'ts']},
    ...
    return <motion.section initial={{opacity: 0}} animate={{opacity: 1}} className="scroll-mt-16" id='projects'>
      {/* <div className='flex  items-c w-full'> */}
        <h2 className="text-4xl lg:text-3xl sm:text-2xl mb-3 lg:mb-1 sm:mb-0">{t('title')}</h2>
        <label className='dark:bg-white/95 text-center ml-auto bg-accent-orange text-white dark:text-black/90 h-min flex-grow-0 lg:text-[10px] sm:text-[8px]'>{t('description')}</label>
      {/* </div> */}
        <motion.ul variants={container} initial="hidden" whileInView="visible" viewport={{once: true}} className='grid grid-cols-3 gap-[60px] my-[40px] dots lg:grid-cols-1'>
